...
function memoriaRevelar() {
  ...
  const cuentaInt = setInterval(() => {
    cuenta--;
    if (cuenta <= 0) {
      clearInterval(cuentaInt);
      memBloqueado = false;
      memoriaRender();
      memoriaIniciarTimer();
    } else {
...
function memoriaClickCarta(pos) {
  ...
  if (memVolteadas.length === 2) {
    ...
    if (memCartas[a].emoji === memCartas[b].emoji) {
      ...
      // error — mostrar en rojo, luego voltear boca abajo (sin cambiar posición)
      memoriaRender({ [a]: 'error', [b]: 'error' });
      setTimeout(() => {
        memBarajadas++;
        memVolteadas = []; // voltear boca abajo — posiciones intactas
...
function memoriaPerder(motivo) {
  ...
  Coins.restar(multa);
  Coins.init();
  guardarDato('Memoria: partidas fallidas',
    (parseInt((JSON.parse(localStorage.getItem('datos_conductuales')||'{}')||{})['Memoria: partidas fallidas']||0)+1)
  );
