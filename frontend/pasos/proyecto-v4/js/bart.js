...
function bartRender() {
  ...
  const r   = 28 + pct * 80;
  const barColor = pct < 0.4 ? '#50fa7b' : pct < 0.7 ? '#f0c040' : '#e63946';
  ...
  P.innerHTML = `
    ...
    <div style="font-family:var(--mono);font-size:0.6rem;color:var(--muted)">
      ${g.label} · explota entre ${g.min}–${g.max} infladas
    </div>
    ...
    <div class="globo-escena">
      <svg width="${Math.max(r*2+24,120)}" height="${Math.max(r*2+50,140)}"
        ...
        ${pct > 0.7 ? `<text x="${r+12}" y="${r+16}" text-anchor="middle" font-size="${r*0.35}" fill="rgba(255,255,255,0.6)">⚠️</text>` : ''}
      </svg>
    </div>

    <div style="width:100%;display:flex;flex-direction:column;gap:4px">
      <div style="font-family:var(--mono);font-size:0.5rem;color:var(--muted);display:flex;justify-content:space-between">
        <span>riesgo</span><span>${Math.round(pct*100)}%</span>
      </div>
      <div class="globo-track">
        <div class="globo-track-fill" style="width:${pct*100}%;background:${barColor}"></div>
      </div>
    </div>
